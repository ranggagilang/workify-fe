...
function SettingPageContent() {
  ...
  const fetchTemplates = async () => {
      try {
          const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/letters`, getHeaders());
  ...
  const handleSaveLetter = async (e: React.FormEvent) => {
      ...
      setIsSaving(true);
      try {
          await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/letters`, {
          ...
          fetchTemplates();
      } catch (e) { alert("Gagal upload template."); } finally { setIsSaving(false); }
  };

  const handleDeleteLetter = async (id: number) => {
      if(!confirm("Yakin ingin menghapus template ini?")) return;
      try {
          await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/letters/${id}`, getHeaders());
          fetchTemplates();
  ...
  const handleSaveProfile = async () => {
      ...
      try {
          ...
          const res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/settings/profile`, payload, getHeaders());
          ...
          fetchSettings();
      } catch (err: any) { alert("Gagal update profile."); } finally { setIsSaving(false); }
  };

  const handleProfilePicChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || file.size > 2 * 1024 * 1024) return alert(`⚠️ Maksimal 2MB.`);
    const reader = new FileReader();
    reader.readAsDataURL(file);
    reader.onloadend = async () => {
        ...
        try {
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/settings/avatar`, { imageBase64: base64String }, getHeaders());
            window.dispatchEvent(new Event('userUpdated'));
  ...
  const handleGetMyLocation = () => {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((pos) => {
                setCompanyForm({ ...companyForm, latitude: pos.coords.latitude.toString(), longitude: pos.coords.longitude.toString() });
  ...
  const handleSaveCompany = async () => {
    setIsSaving(true);
    try {
        await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/companies/profile`, companyForm, getHeaders());
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                   <div className="space-y-6">
                       ...
                       <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                          ...
                          <div><p className="text-sm font-bold text-gray-700">Logo Perusahaan</p></div>
                       </div>
                       <input type="text" className="w-full border p-2 rounded-lg" placeholder="Company Name" value={companyForm.name} onChange={(e)=>setCompanyForm({...companyForm, name: e.target.value})}/>
                       <input type="text" className="w-full border p-2 rounded-lg" placeholder="Phone" value={companyForm.phone} onChange={(e)=>setCompanyForm({...companyForm, phone: e.target.value})}/>
                       <textarea className="w-full border p-2 rounded-lg" placeholder="Address" rows={3} value={companyForm.address} onChange={(e)=>setCompanyForm({...companyForm, address: e.target.value})}/>
                   </div>
                   <div className="space-y-6">
                       ...
                       <button type="button" onClick={handleGetMyLocation} className="w-full py-2 bg-gray-800 text-white rounded-lg text-sm font-bold hover:bg-gray-900 flex justify-center items-center gap-2">📍 Ambil Lokasi Sekarang</button>
        ...
        {activeTab === 'letters' && (
             <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-[#19A0FA]/20 shadow-sm">
                    <div><h1 className="text-xl font-bold text-gray-800">Manajemen Template Surat</h1></div>
                    <button onClick={()=>setIsLetterModalOpen(true)} className="px-4 py-2 bg-[#19A0FA] text-white rounded-lg font-bold flex gap-2 items-center hover:bg-blue-600 transition"><HiPlus className="w-5 h-5"/> Upload Template</button>
                </div>
                    ...
                    {templates.map((item) => (
                        <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 flex justify-between items-center shadow-sm hover:shadow-md transition">
                            <div className="flex gap-3 items-center">
                                ...
                                <div><h4 className="font-bold text-gray-800">{item.title}</h4></div>
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => handleDownload(item.fileUrl, item.title)} className="p-2 text-gray-400 hover:text-blue-500"><HiArrowDownTray className="w-5 h-5"/></button>
                                <button onClick={()=>handleDeleteLetter(item.id)} className="p-2 text-gray-400 hover:text-red-500"><HiTrash className="w-5 h-5"/></button>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      {isLetterModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
              <div className="bg-white p-6 rounded-xl w-full max-w-md">
                  <h3 className="font-bold text-lg mb-4">Upload Template</h3>
                  <form onSubmit={handleSaveLetter} className="space-y-4">
                      <input className="w-full p-2 border rounded-lg" placeholder="Judul" value={letterForm.title} onChange={e=>setLetterForm({...letterForm, title: e.target.value})} required/>
                      <div onClick={()=>letterInputRef.current?.click()} className="border-2 border-dashed p-6 text-center cursor-pointer rounded-xl">
                          {letterForm.fileBase64 ? "✅ File Siap" : "Klik untuk Upload PDF"}
                          <input ref={letterInputRef} type="file" accept=".pdf,.docx" className="hidden" onChange={handleLetterFileChange} />
                      </div>
                      <div className="flex gap-2">
                          <button type="button" onClick={()=>setIsLetterModalOpen(false)} className="flex-1 py-2 text-gray-500">Batal</button>
                          <button type="submit" disabled={isSaving} className="flex-1 py-2 bg-[#19A0FA] text-white rounded-lg">{isSaving ? "Menyimpan..." : "Simpan"}</button>
                      </div>
                  </form>
              </div>
          </div>
...
// --- PEMBUNGKUS SUSPENSE (SYARAT VERCEL) ---
export default function SettingPage() {
  return (
    <Suspense fallback={<div className="p-10 text-center text-[#19A0FA] font-bold">Loading...</div>}>
      <SettingPageContent />
    </Suspense>
  );
}
...
function EditableItem({ label, name, value, isEditing, onChange }: any) {
    return (
      <div className="group">
        ...
        {isEditing ? (
            <input name={name} value={value || ''} onChange={onChange} className="w-full text-base text-gray-800 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-[#19A0FA] transition-all"/>
        ) : (
...
function CountryEditableItem({ label, name, value, isEditing, onChange }: any) {
  return (
    <div className="group">
      <p className="text-xs font-bold uppercase tracking-wide mb-1 text-gray-500">{label}</p>
      {isEditing ? (
          <select name={name} value={value || ''} onChange={onChange} className="w-full text-base text-gray-800 border border-gray-300 rounded-lg px-3 py-2 bg-white">
...
function TimelineItem({ time, title, icon: Icon, children }: any) {
  return (
    <div className="relative pl-8">
      <span className="absolute -left-3.5 top-0 flex items-center justify-center w-7 h-7 bg-white border-2 border-[#19A0FA] rounded-full z-10"><Icon className="w-3.5 h-3.5 text-[#19A0FA]" /></span>
      <div>
...
function NotificationItem({ label, description, isActive, onToggle }: any) {
  return (
    <div className="flex items-center justify-between p-4 border border-gray-100 rounded-xl bg-white">
      ...
      <button onClick={onToggle} className={`w-12 h-6 flex items-center rounded-full p-1 transition-colors ${isActive ? 'bg-[#19A0FA]' : 'bg-gray-300'}`}><span className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${isActive ? 'translate-x-6' : 'translate-x-0'}`} /></button>
    </div>
  );
}